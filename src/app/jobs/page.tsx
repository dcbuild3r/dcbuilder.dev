...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (jobId) {
    const job = await withDataFallback('jobs.metadata', getPublicJobById(jobId), null);
    if (job) {
...
const getPublicJobTaxonomy = cachePublicData(
  ['job-taxonomy'],
  async () => {
    const [tags, roles] = await Promise.all([getJobTagsWithFallback(), getJobRolesWithFallback()]);
    return { tags, roles };
  },
  ['jobs']
);
