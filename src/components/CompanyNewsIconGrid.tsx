...
export function CompanyNewsIconGrid({
  ...
  return (
    <section aria-labelledby="company-news-icon-grid-heading" className={sectionClassName}>
      <div
        className={
          isCompact
          ...
          {companies.map((company) => {
            ...
            return (
              <Link
                ...
                title={company.name}
                className={`${linkClassName} ${isActive ? 'scale-105' : ''}`}
              >
                <Image
                  src={company.logo}
                  alt=""
                ...
                <span className="sr-only">{company.newsCount} news items</span>
              </Link>
            );
          })}
        </div>
