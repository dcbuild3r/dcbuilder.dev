...
export function CandidatesGrid({ candidates }: CandidatesGridProps) {
  ...
  const replaceSearchParams = useCallback((nextParams: URLSearchParams) => {
    ...
    window.history.replaceState(
      null,
      '',
      queryString ? `${currentUrl.pathname}?${queryString}` : currentUrl.pathname
    );
  }, []);
  ...
  const getCandidateEventProps = useCallback(
    (candidate: Candidate) => ({
  ...
  const filteredCandidates = useMemo(() => {
    return candidates.filter((candidate) => {
      ...
      } else if (availabilityFilter !== 'all' && candidate.availability !== availabilityFilter) {
        return false;
      }

      // Experience filter
      if (experienceFilter !== 'all' && candidate.experience !== experienceFilter) {
        return false;
      }
      ...
      if (selectedTags.length > 0) {
        if (!candidate.skills || !selectedTags.every((tag) => candidate.skills?.includes(tag))) {
          return false;
        }
      }
  ...
  return (
    <div className="space-y-6" data-testid="candidates-grid" data-hydrated={hydrated}>
      {/* Filters */}
...
const CandidateCard = memo(function CandidateCard({
  ...
  const displayName = isAnonymous ? candidate.anonymousAlias || 'Anonymous' : candidate.name;
  ...
  const cardStyle = isHot ? CARD_STYLES.hot : isTopStyle ? CARD_STYLES.top : CARD_STYLES.default;

  return (
    <div
      data-testid="candidate-card"
      className={cn(
        'p-4 rounded-xl border transition-[border-color,box-shadow,background-color] overflow-hidden',
        cardStyle
      )}
    >
      {/* Header */}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs text-neutral-500">
        ...
        {candidate.remote && <span className="text-green-600 dark:text-green-400">Remote OK</span>}
      ...
      <div className="mt-3 min-h-[52px] flex flex-wrap items-start justify-center gap-1.5 overflow-hidden">
        {(() => {
          const displaySkills = (candidate.skills || []).filter(
            (tag) => tag !== 'hot' && tag !== 'top'
          );
          const maxTags = 4;
