...
date: "2021-11-14"
description: "A comprehensive guide to Layer 2 scaling solutions on Ethereum, covering optimistic rollups, zk-rollups, validiums, and the future of Ethereum scalability."
source: "Mirror"
sourceUrl: "https://paragraph.com/@mirror.dcbuilder/the-ultimate-guide-to-l2s-on-ethereum"
---

![Cover photo](https://img.paragraph.com/cdn-cgi/image/format=auto,width=3840,quality=85/https://storage.googleapis.com/papyrus_images/b2c7bcd4cc16820b688515f4a66956edd8bb88ee0ecf12ee2b8bd0b93e83b2c6.png)
...
- The application design space is growing now that builders have more bandwidth and fewer execution layer hurdles to work with
...
![Scaling solution categorization](https://img.paragraph.com/cdn-cgi/image/format=auto,width=640,quality=85/https://storage.googleapis.com/papyrus_images/cd17260426885ed8444c08acf2345ec761e3d9310bf106d197c3ac1493a1f7d5.png)
...
For a more complete explanation of why rads are the only solution for long-term scaling read [@polynya's articles](https://polynya.medium.com/) in the 'Further reading' section.
...
There are many protocols and applications that already support Arbitrum, including supporting infrastructure that makes the switch to using Arbitrum layer 2 almost seamless. The only current issue with using L2s is that it needs to accrue more liquidity and innovative solutions like cross-L2 AMM structures like [dAMM](https://medium.com/starkware/damm-decentralized-amm-59b329fb4cc3) (invented by Starkware and Loopring) and liquidity protocols like Connext and Hop needs to get enough liquidity and become trustless enough to the point that all of these scaling solutions can share the same infrastructure so as to not cause ecosystem fragmentation.
...
If using the Arbitrum bridge is confusing, check out the [Arbitrum bridge tutorial](https://arbitrum.io/bridge-tutorial/). For bridging from other networks besides Ethereum check out the L2 bridges section below (note that these bridges all have varying degrees of centralization).
...
- different fault proof verification mechanism
...
- the fault proof infrastructure
- stateless clients
- sharding
...
StarkEx generates validity proofs which ensure that all the off-chain computations were performed with integrity, the STARK proof generated then gets verified on-chain before getting committed to the Ethereum mainnet.
...
There are additional features that will be rolled out as part of Alpha 1 and Alpha 2 which include:
...
- Cairo audit services: In the coming months, ABDK, ConsenSys Diligence, Peckshield, and Trail of Bits will be conducting the Cairo audit
...
![Polygon Hermez tentative release schedule](https://img.paragraph.com/cdn-cgi/image/format=auto,width=640,quality=85/https://storage.googleapis.com/papyrus_images/5318972ed99919d4b43ab0d2481fb17b8ed27a850084934ad315c3f0840dcfde.png)
...
[Loopring](https://loopring.org/#/) protocol is an open-source zkRollup protocol. It is a collection of Ethereum smart contracts and ZK circuits which describe how to build highly secure, highly-scalable order book-based DEXes, AMMs, and payment apps.
...
[Sorare](https://sorare.com/) is a fantasy football game that utilizes a StarkEx Validium to scale its NFT game for the masses.
...
"[deBridge](https://debridge.finance/) is a cross-chain interoperability and liquidity transfer protocol that allows the truly decentralized transfer of arbitrary data and assets between various blockchains. The cross-chain intercommunication of deBridge smart contracts is powered by the network of independent oracles/validators which are elected by deBridge governance."
...
![dAMM architecture](https://img.paragraph.com/cdn-cgi/image/format=auto,width=640,quality=85/https://storage.googleapis.com/papyrus_images/706b566bc317bbdd696f55f92b2109030f7e52e603f84fd41707c4fdcb5e7bb1.png)
...
These are tools I use on a daily basis to gauge the state of adoption of Ethereum L2s, how expensive transactions on their cost, and what is the L2 protocol revenue.

#### Dune Analytics

[Dune Analytics](https://dune.xyz/browse/dashboards) is a data analytics platform that allows anyone to easily aggregate and visualize blockchain data. On Dune you can create a data hub for your research project, article, DAO, or any other project in a matter of hours. With regards to L2s, I use it to monitor how many funds are locked inside of L2 bridges. Relevant data dashboards:

- [Bridge Away dashboard](https://dune.xyz/eliasimos/Bridge-Away-\(from-Ethereum\)) created by [@eliasimos](https://twitter.com/eliasimos) provides an overview of how many funds are locked inside bridges into other L1s (Avalanche, BSC, Fantom, …), sidechains (Ronin, Polygon PoS), and also several L2s like (Arbitrum, Optimism, zkSync, and Boba). It is also good to know where users are bridging to see where is capital escaping to know where Ethereum is lacking and how it can improve. If Ethereum provides a better UX, then much less capital will leave the network (especially for new users).
...
![L2BEAT L2 TVL](https://img.paragraph.com/cdn-cgi/image/format=auto,width=640,quality=85/https://storage.googleapis.com/papyrus_images/423950a5a063a62022c20d772136de14413d28569cc1c3ff5999ceddd2761fd2.png)

L2BEAT L2 TVL

![L2BEAT project dashboard](https://img.paragraph.com/cdn-cgi/image/format=auto,width=640,quality=85/https://storage.googleapis.com/papyrus_images/d9cebb91da26ea08f8cd89290a82928eb46c07bd3b8b523d8e47a222a6754aa3.png)
...
![L2Fees comparison](https://img.paragraph.com/cdn-cgi/image/format=auto,width=640,quality=85/https://storage.googleapis.com/papyrus_images/9a05d007b065256636f101f0adae32062918e5d244cdafb6df0d88fff3dea690.png)
...
[Nansen](https://www.nansen.ai/) is a blockchain analytics platform that enriches on-chain data with millions of wallet labels. It is a paid platform and isn't exactly cheap. However, it is very affordable for the amount of value it provides in return if used correctly. I personally use Nansen for identifying narratives within DeFi and NFTs, however, I've recently started to use their wallet profiler feature on the smart contracts for L2s to see which entities are bridging to L2s and how fast. It shows a much clearer picture as many addresses and active players are labeled.

**Chainlist**

[Chainlist](https://chainlist.org/) is an application that contains the RPC configurations needed to use a certain network inside Metamask or other web3 wallets. You can add different networks that are EVM compatible - L1s, sidechains, and also L2s. Relevant L2 networks available on Chainlist are Optimistic Ethereum mainnet, Arbitrum One, Boba Network, and others soon to come.

My article can only be so long, it already is quite long and it wouldn't make sense to copy-paste other people's work. There is a myriad of quality learning resources on L2s that can take your understanding to a whole new level. I'll try to link as many of these as I can, if you have any suggestions, DM me on [Twitter](https://twitter.com/DCbuild3r) or Telegram @dcbuild3r.
...
- [polynya's Medium posts](https://t.co/A4qTQ0cxYt?amp=1) - [@polynya](https://twitter.com/apolynya) (articles)
...
- [The Daily Gwei](https://thedailygwei.substack.com/) - Anthony Sassano ([newsletter](https://thedailygwei.substack.com/), [YT channel](https://www.youtube.com/c/TheDailyGwei))
...
This article is a compilation of my current understanding of L2s on Ethereum, I'll try to keep this article updated as best I can, but for that, I'm also going to need the help of the community. This article is available as a [GitHub repository](https://github.com/dcbuild3r/web3-articles/tree/main/Waifus%20Anonymous/L2s), so if you think that any of the information presented is inaccurate, please submit a pull request or DM me with what's wrong and I'll try my best to fix it. I'm not an expert in cryptography or L2 technology, so if my understanding is incorrect, please correct me.
